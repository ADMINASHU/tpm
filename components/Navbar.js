...
import {
    LayoutDashboard, PackageSearch, Wrench, Truck,
    LineChart, Settings, LogOut, FileBadge, BookOpen,
...
const navItems = [
    { name: "Dashboard", href: "/dashboard", icon: LayoutDashboard },
    {
        ...
        children: [
            { name: "Create Indent", href: "/procurement?tab=indent", icon: ClipboardList },
            { name: "PO Gen & Approvals", href: "/procurement?tab=po", icon: FileCheck },
            { name: "Supplier Config", href: "/procurement?tab=suppliers", icon: Factory },
        ],
    },
    {
    ...
    },
    {
        ...
        children: [
            { name: "New Assembly Build", href: "/production?tab=build", icon: Layers },
            { name: "BOM Configuration", href: "/production?tab=bom", icon: GitBranch },
            { name: "Genealogy Trace", href: "/production?tab=genealogy", icon: QrCode },
            { name: "Product Config", href: "/production?tab=products", icon: Package },
            { name: "Component Config", href: "/production?tab=components", icon: Settings },
            { name: "Spare Parts Config", href: "/production?tab=spares", icon: Wrench },
        ],
    },
    {
        name: "Logistics", href: "/logistics", icon: Truck,
        children: [
            { name: "Stock Transfer", href: "/logistics?tab=transfer", icon: RefreshCw },
            { name: "External Dispatch", href: "/logistics?tab=dispatch", icon: TruckOut },
        ],
    },
    {
...
function AvatarMenu({ session }) {
    ...
    return (
        <div ref={ref} className="relative">
            ...
            >
                {/* Avatar circle only */}
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white text-xs font-black shadow-sm ring-2 ring-white">
                    {initials}
                </div>
            ...
            {open && (
                <div className="absolute top-full right-0 mt-2 w-60 bg-white rounded-2xl shadow-xl border border-slate-100 ring-1 ring-slate-100/50 z-50 overflow-hidden">
                    {/* User header */}
                    <div className="px-4 py-4 bg-gradient-to-br from-indigo-50 to-slate-50 border-b border-slate-100">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white text-sm font-black shadow">
                                {initials}
                            </div>
                            <div>
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <div className="hidden md:flex items-center gap-1 mr-2">
                                {navItems.map((item) => {
                                    ...
                                    if (item.children) {
                                        return <NavDropdown key={item.name} item={item} isActive={isActive} />;
                                    }
