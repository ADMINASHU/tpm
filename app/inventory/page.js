...
function InventoryContent() {
    ...
    useEffect(() => {
        if (["grn", "hex", "stock", "tags"].includes(tabFromUrl)) setActiveTab(tabFromUrl);
    }, [tabFromUrl]);
            ...
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="md:hidden flex space-x-1 p-1 bg-slate-200/50 rounded-xl w-fit">
                    <button onClick={() => setActiveTab("grn")}
                        className={`px-6 py-2.5 text-sm font-semibold rounded-lg transition-all ${activeTab === "grn" ? "bg-white text-indigo-700 shadow-sm ring-1 ring-slate-200" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/50"
                            }`}>Quick GRN</button>
                    <button onClick={() => setActiveTab("hex")}
                        className={`px-6 py-2.5 text-sm font-semibold rounded-lg transition-all ${activeTab === "hex" ? "bg-white text-indigo-700 shadow-sm ring-1 ring-slate-200" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/50"
                            }`}>Hex Tag Indexing</button>
                </div>
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-slate-50 border border-slate-200 p-4 rounded-xl">
                <p className="text-sm font-semibold text-slate-600">Total Unique Items: <span className="font-bold text-slate-900">142</span></p>
                <div className="relative w-64">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                    <input type="text" className="block w-full pl-9 py-2 rounded-lg border-0 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-200 focus:ring-2 focus:ring-indigo-600 sm:text-sm" placeholder="Search inventory..." />
                </div>
            </div>
            <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-50">
                    <tr className="hover:bg-slate-50"><td className="py-3 px-4 font-mono font-bold text-indigo-600">RES-10K-001</td><td className="py-3 px-4 font-semibold text-slate-900">10k Ohm Resistor 1/4W</td><td className="py-3 px-4 text-slate-500">Passives</td><td className="py-3 px-4 text-right font-bold text-emerald-600">5,430</td></tr>
                    <tr className="hover:bg-slate-50"><td className="py-3 px-4 font-mono font-bold text-indigo-600">CAP-22U-002</td><td className="py-3 px-4 font-semibold text-slate-900">22uF Electrolytic Cap</td><td className="py-3 px-4 text-slate-500">Passives</td><td className="py-3 px-4 text-right font-bold text-emerald-600">1,200</td></tr>
                    <tr className="hover:bg-slate-50"><td className="py-3 px-4 font-mono font-bold text-indigo-600">IC-MCU-AT32</td><td className="py-3 px-4 font-semibold text-slate-900">ATmega328P Microcontroller</td><td className="py-3 px-4 text-slate-500">Active ICs</td><td className="py-3 px-4 text-right font-bold text-amber-600">120</td></tr>
                </tbody>
            </table>
        </div>
    );
        ...
        <div className="max-w-2xl space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
                <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-2xl">
                    <h3 className="font-bold text-indigo-900 mb-2">Bulk Empty Hex Tags</h3>
                    <p className="text-sm text-indigo-700/80 mb-4">Print sheets of blank 4-digit hex QR codes for incoming GRN mapping.</p>
                    <input type="number" defaultValue="50" className="block w-32 rounded-lg border-0 py-2.5 px-3 mb-4 shadow-sm ring-1 ring-inset ring-slate-200 text-sm" />
                    <button className="bg-indigo-600 text-white font-bold text-sm px-4 py-2 rounded-xl hover:bg-indigo-700 w-full">Print Empty Hex Tags</button>
                </div>
                <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl">
                    <h3 className="font-bold text-slate-900 mb-2">Specific Component Tags</h3>
                    <p className="text-sm text-slate-500 mb-4">Print labels mapped to a specific known component code.</p>
                    <input type="text" placeholder="Component Code" className="block w-full rounded-lg border-0 py-2.5 px-3 mb-4 shadow-sm ring-1 ring-inset ring-slate-200 text-sm" />
                    <button className="bg-slate-800 text-white font-bold text-sm px-4 py-2 rounded-xl hover:bg-slate-700 w-full">Print Direct Tags</button>
                </div>
            </div>
        </div>
    );
