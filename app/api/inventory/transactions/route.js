...
export async function GET(req) {
    ...
    const Types = dbConnect.mongoose?.Types || (await import("mongoose")).default.Types;
    ...
    if (summary === "true") {
        ...
        if (ids) {
            ...
            // We need to compare against ObjectId if they are ObjectIds, or string if string.
            // In Mongo, ids in DB might be ObjectIds, so we need to map them.
            const Types = dbConnect.mongoose?.Types || (await import("mongoose")).default.Types;
        ...
        try {
            const stats = await InventoryTransaction.aggregate([
                { $match: match },
                {
                    $group: {
                        _id: {
                            configId: "$configId", // Group by the master config
                            item: "$item", // Still keep link to item for bulk items
                            product: "$product"
                        },
            ...
            // To avoid double counting or fragmentation, we'll refine the aggregation to group primarily by Config
            const refinedStats = await InventoryTransaction.aggregate([
                ...
                },
                {
                    $addFields: {
                        ...
                        // Flag inbound transactions that carry a meaningful unitPrice
                        isInbound: {
                            $and: [
                                { $in: ["$type", ["GRN", "OPENING_STOCK", "STOCK_TRANSFER_IN", "PRODUCTION_OUTPUT"]] },
                                { $gt: ["$unitPrice", 0] }
                            ]
                        }
                    }
                },
                {
                    $group: {
                        _id: {
                            ...
                            fallbackId: { $cond: ["$resolvedConfigId", null, { $ifNull: ["$item", "$product"] }] }
                        },
                        totalQty: { $sum: "$quantity" },
                        // Weighted cost numerator: sum of (unitPrice * qty) for inbound transactions only
                        totalCostValue: {
                            $sum: {
                                $cond: [
                                    "$isInbound",
                        ...
                        // Weighted cost denominator: sum of qty for inbound transactions only
                        totalCostQty: {
                            $sum: {
            ...
            const populated = await Promise.all(refinedStats.map(async (stat) => {
                ...
                if (cid) {
                    if (model === "ComponentConfig") {
                        const conf = await (await import("@/lib/models/ComponentConfig")).default.findById(cid).lean();
                        if (conf) details = { ...details, ...conf, type: 'item' };
                    } else if (model === "SpareConfig") {
                        const conf = await (await import("@/lib/models/SpareConfig")).default.findById(cid).lean();
                        if (conf) details = { ...details, ...conf, type: 'item' };
                    } else if (model === "ProductConfig") {
                        const conf = await (await import("@/lib/models/ProductConfig")).default.findById(cid).lean();
                        if (conf) details = { ...details, ...conf, itemName: conf.productName, itemCode: conf.serialNumber, type: 'product' };
                    }
                } else if (fid) {
                    // Fallback to Item/Product for legacy data
                    const item = await Item.findById(fid).lean();
                    if (item) {
                        details = { ...details, ...item, type: 'item' };
                    } else {
                        const prod = await (await import("@/lib/models/FinishedProduct")).default.findById(fid).lean();
                        if (prod) details = { ...details, ...prod, itemName: prod.productName, itemCode: prod.serialNumber, type: 'product' };
                    }
                }

                return {
                    ...details,
                    _id: cid || fid, // Add unique ID for React keys
                    currentQuantity: stat.totalQty, // Always use the aggregated qty
                    averageUnitCost: stat.averageUnitCost || 0, // Always use the computed weighted avg cost
                    estimatedTotalValue: (stat.totalQty || 0) * (stat.averageUnitCost || 0),
                };
            }));
    ...
    try {
        ...
        const resolved = await Promise.all(transactions.map(async (tx) => {
            ...
            const cmodel = tx.configModel || tx.item?.configModel || (tx.product ? "ProductConfig" : null);

            if (cid && cmodel) {
                try {
                    if (cmodel === "ComponentConfig") {
                        const conf = await (await import("@/lib/models/ComponentConfig")).default.findById(cid).lean();
                        if (conf) { masterName = conf.itemName; masterCode = conf.itemCode; }
                    } else if (cmodel === "SpareConfig") {
                        const conf = await (await import("@/lib/models/SpareConfig")).default.findById(cid).lean();
                        if (conf) { masterName = conf.itemName; masterCode = conf.itemCode; }
                    } else if (cmodel === "ProductConfig") {
                        const conf = await (await import("@/lib/models/ProductConfig")).default.findById(cid).lean();
                        if (conf) { masterName = conf.productName; masterCode = conf.serialNumber; }
                    }
                } catch (e) {
            ...
            return {
                ...tx,
                masterName: masterName || tx.item?.itemName || tx.product?.productName || "Unknown",
                masterCode: masterCode || tx.item?.itemCode || tx.product?.serialNumber || "N/A"
            };
        }));
...
export async function POST(req) {
    ...
    try {
        ...
        const { itemId, productId, configId, configModel, type, quantity, unitPrice, notes, reference, entityTag, date } = body;
        const factoryId = session.user.factoryId;

        if ((!itemId && !productId && !configId) || !type || quantity === undefined) {
        ...
        if (configId) {
            ...
            if (configModel === "ProductConfig") {
                ...
                let prod = await (await import("@/lib/models/FinishedProduct")).default.findOne({
                    configId: configId,
                    serialNumber: entityTag
                });

                if (!prod) {
                    const conf = await (await import("@/lib/models/ProductConfig")).default.findById(configId);
                    if (!conf) return NextResponse.json({ error: "Product Config not found" }, { status: 404 });

                    prod = await (await import("@/lib/models/FinishedProduct")).default.create({
                ...
                // Component or Spare
                const conf = configModel === "SpareConfig"
                    ? await (await import("@/lib/models/SpareConfig")).default.findById(configId)
                    : await (await import("@/lib/models/ComponentConfig")).default.findById(configId);

                if (!conf) return NextResponse.json({ error: "Configuration not found" }, { status: 404 });
